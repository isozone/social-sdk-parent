<template>
  <div class="page-root">
    <el-card shadow="never">
      <div class="card-head">
        <div class="card-head-left">
          <div class="card-chip chip-purple"><el-icon><ChatLineSquare /></el-icon></div>
          <div class="card-head-text">
            <div class="card-title">商品专属回复</div>
            <div class="card-sub">指定商品在触发场景下回复固定内容，优先级高于通用关键词回复（BOT-D1）</div>
          </div>
        </div>
        <el-button type="primary" @click="openCreate"><el-icon><Plus /></el-icon> 新建回复</el-button>
      </div>
      ...
      <el-form inline style="margin-top: 12px">
        ...
        <el-form-item label="场景">
          <el-select v-model="filter.triggerScene" placeholder="全部场景" clearable style="width: 180px" @change="loadList">
            <el-option label="首次询单（FIRST_INQUIRY）" value="FIRST_INQUIRY" />
            <el-option label="拍下（ORDER_PLACED）" value="ORDER_PLACED" />
            <el-option label="付款（ORDER_PAID）" value="ORDER_PAID" />
          </el-select>
        </el-form-item>
        <el-form-item>
      ...
      <el-table :data="rows" v-loading="loading" border stripe style="margin-top: 8px">
        ...
        <el-table-column prop="triggerScene" label="场景" width="140" />
        <el-table-column prop="replyContent" label="回复内容" min-width="280" show-overflow-tooltip />
    ...
    <el-dialog v-model="dialogVisible" :title="editing.id ? '编辑回复' : '新建回复'" width="560px">
      <el-form :model="editing" label-width="100px">
        <el-form-item label="账号">
          <el-select v-model="editing.accountId" placeholder="必选账号" filterable style="width: 100%">
            <el-option v-for="a in accounts" :key="a.id" :label="a.accountName || a.id" :value="a.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="商品ID">
          <el-input v-model="editing.itemId" placeholder="闲鱼 itemId" />
        </el-form-item>
        <el-form-item label="商品标题">
          <el-input v-model="editing.itemTitle" placeholder="冗余字段，便于管理端识别" />
        </el-form-item>
        <el-form-item label="触发场景">
          <el-select v-model="editing.triggerScene" style="width: 100%">
            <el-option label="首次询单（FIRST_INQUIRY）" value="FIRST_INQUIRY" />
            <el-option label="拍下（ORDER_PLACED）" value="ORDER_PLACED" />
            <el-option label="付款（ORDER_PAID）" value="ORDER_PAID" />
          </el-select>
        ...
        <el-form-item label="回复内容">
          <el-input v-model="editing.replyContent" type="textarea" :rows="4" placeholder="支持占位符：{商品名} {买家昵称}" />
        </el-form-item>
...
import { listItemReplies, createItemReply, updateItemReply, deleteItemReply, toggleItemReplyEnabled } from '@/api/bot.js'
...
const size = ref(20)
const total = ref(0)
const filter = reactive({ accountId: null, itemId: '', triggerScene: '' })
...
async function loadList() {
  loading.value = true
  try {
    const res = await listItemReplies({ page: page.value, size: size.value, accountId: filter.accountId, itemId: filter.itemId, triggerScene: filter.triggerScene })
...
function openCreate() {
  Object.assign(editing, { id: null, accountId: null, itemId: '', itemTitle: '', triggerScene: 'FIRST_INQUIRY', replyContent: '', priority: 100, enabled: 1, remark: '' })
...
async function onSave() {
  if (!editing.accountId || !editing.itemId || !editing.replyContent) { ElMessage.warning('账号、商品ID、回复内容必填'); return }
  saving.value = true
  try {
    const res = editing.id ? await updateItemReply(editing.id, editing) : await createItemReply(editing)
...
async function onToggle(row) {
  try {
    const res = await toggleItemReplyEnabled(row.id, row.enabled !== 1)
...
async function onDelete(id) {
  try {
    const res = await deleteItemReply(id)
