    ...
    <el-card shadow="never" style="margin: 0;">
      <div class="card-head">
        <div class="card-head-left">
          <div class="card-chip chip-slate"><el-icon><Avatar /></el-icon></div>
          <div class="card-head-text">
            <div class="card-title">登录续期日志</div>
            <div class="card-sub">追踪登录续期（A3）批次的执行结果、等待扫码与失败原因</div>
          </div>
        </div>
        <div class="card-head-right">
          <el-button size="small" @click="handleRun" :loading="running" title="手动触发一次登录续期批次">
            <el-icon><VideoPlay /></el-icon>&nbsp;手动续期
          </el-button>
      ...
      <el-table :data="records" stripe v-loading="loading" empty-text="暂无数据">
        ...
        <el-table-column prop="waitingQrCount" label="等待扫码" width="90" align="center" />
...
import { Refresh, VideoPlay, Avatar } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { listLoginRenewLogs, runLoginRenew } from '@/api/renew'
...
async function loadData() {
  loading.value = true
  try {
    const res = await listLoginRenewLogs({ page: pageNum.value, size: pageSize.value })
    if (res.success) {
      records.value = res.data.records || []
      total.value = res.data.total || 0
    }
  } catch (e) {
    ElMessage.error('加载登录续期日志失败：' + (e.message || '未知错误'))
  } finally {
    loading.value = false
  }
}
...
async function handleRun() {
  running.value = true
  try {
    const res = await runLoginRenew()
    if (res.success) {
      ElMessage.success('已触发登录续期批次')
      loadData()
    } else {
