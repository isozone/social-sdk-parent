...
-- ALTER TABLE xianyu_order ADD COLUMN virtual_shipped_at TIMESTAMP;
-- ALTER TABLE xianyu_order ADD COLUMN auto_receipt_at TIMESTAMP;
...
CREATE TABLE IF NOT EXISTS virtual_ship_config (
    ...
    notify_after_ship BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    deleted INTEGER DEFAULT 0,
    UNIQUE(account_id),
...
CREATE INDEX IF NOT EXISTS idx_market_keyword_keyword ON market_keyword(keyword);
CREATE INDEX IF NOT EXISTS idx_market_keyword_status ON market_keyword(status);
...
CREATE TABLE IF NOT EXISTS login_renew_schedule (
    ...
    updated_at          TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    deleted             INTEGER DEFAULT 0,
    CONSTRAINT uk_login_renew_account UNIQUE (account_id)
);
...
CREATE TABLE IF NOT EXISTS im_token_cache (
    ...
    updated_at              TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    deleted                 INTEGER DEFAULT 0,
    CONSTRAINT uk_im_token_account UNIQUE (account_id)
);
...
CREATE TABLE IF NOT EXISTS card_item_relation (
    ...
    updated_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    deleted         INTEGER DEFAULT 0,
    CONSTRAINT uk_card_rel_product_card UNIQUE (product_id, card_id)
);
