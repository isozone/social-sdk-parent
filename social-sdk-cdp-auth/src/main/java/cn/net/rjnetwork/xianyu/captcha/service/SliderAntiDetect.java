...
public final class SliderAntiDetect {
    ...
    public static String buildScript(long seed) {
        ...
        return ""
                ...
                + "    if (!window.chrome.runtime.id) { try { Object.defineProperty(window.chrome.runtime, 'id', { value: 'nmmhkkegccagdldgiimedpiccmgmieda', configurable: false }); } catch (e2) {} }\n"
                ...
                + "      // 从 UA 解析 Chrome 主版本，brands 版本与 UA 对齐（避免 brand 与 UA 版本不一致的机器特征）\n"
                + "      const uaVer = (sanitizedUA.match(/Chrome\\/(\\d+)\\./) || [])[1] || '';\n"
                + "      const align = (b) => {\n"
                + "        const brand = String(b && b.brand || '').replace(/Headless/gi, '');\n"
                + "        let version = String(b && b.version || '');\n"
                + "        if (uaVer && /Chrome/i.test(brand)) { version = uaVer; }\n"
                + "        return { brand, version };\n"
                + "      };\n"
                + "      Object.defineProperty(navigator, 'userAgentData', { get: () => ({\n"
                + "        brands: (uaData.brands || []).map(align),\n"
                ...
                + "          if (values && typeof values === 'object') { values.brands = (values.brands || []).map(align); values.platform = platformName || values.platform || ''; }\n"
                + "          return values;\n"
                + "        }\n"
                + "      }) });\n"
                + "    }\n"
                ...
                + "  } catch (e) {}\n"
                + "\n"
                + "  try { if (navigator.connection) { Object.defineProperty(navigator.connection, 'rtt', { get: () => 50 }); Object.defineProperty(navigator.connection, 'downlink', { get: () => 10 }); Object.defineProperty(navigator.connection, 'effectiveType', { get: () => '4g' }); Object.defineProperty(navigator.connection, 'saveData', { get: () => false }); } } catch (e) {}\n"
                ...
                + "  // ---- WebRTC 指纹：阻止 RTCPeerConnection 暴露本地真实 IP（srflx/host 候选） ----\n"
                + "  try {\n"
                ...
                + "          const typ = idx >= 0 ? parts[idx + 1] : '';\n"
                + "          if (typ === 'srflx' || typ === 'host') { c.candidate = c.candidate.replace(/\\d{1,3}(\\.\\d{1,3}){3}/g, '0.0.0.0'); }\n"
                + "        }\n"
                + "        return c;\n"
                + "      };\n"
                + "      const origAdd = RTCPeerConnection.prototype.addEventListener;\n"
                + "      RTCPeerConnection.prototype.addEventListener = function(type, listener, opts) {\n"
                + "        if (type === 'icecandidate' && listener) {\n"
                + "          const wrapped = (ev) => { try { if (ev && ev.candidate) { ev.candidate = sanitizeCandidate(ev.candidate); } } catch (e3) {} return listener.call(this, ev); };\n"
